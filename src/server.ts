...
const server = app.listen(port, () => {
  console.log(`agent-console-api listening on http://localhost:${port}`);
});

// SSE connections are long-lived, so an unbounded graceful shutdown would hang
// forever. Give clients a moment, then force the process down.
